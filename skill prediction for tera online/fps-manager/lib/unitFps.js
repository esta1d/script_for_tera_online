		...
		const isSkillBlockable = (event) => {
			...
			if (data.activeMode.blockSkills) {
				...
				if (data.activeMode.blockSkills.all)
					...
					return true;
				else if (data.activeMode.blockSkills.rawId && utils.arraysHasIntersect(event.skill.id, data.activeMode.blockSkills.rawId))
					return true;
				else if (data.activeMode.blockSkills.extended && data.activeMode.blockSkills.extended[data.userTemplateToClassMap[event.templateId]])
					if (data.activeMode.blockSkills.extended[data.userTemplateToClassMap[event.templateId]][utils.getSkillBase(event.skill.id)]) //TODO: refactor it
						return true;
			}
			return false;
		};
